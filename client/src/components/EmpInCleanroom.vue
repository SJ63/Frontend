		...
		<div class="row mt-4 g-0">
			<div class="col">
				<table class="table table-striped table-hover table-responsive table-dark">
					...
					<tbody>
						<tr v-for="(item, index) in list_Incleanroom_filter_slice" :key="item.autoID">
							<td>{{ index+1 }}</td>
							<td>{{ item.empId }}</td>
...
export default {
  ...
  data: () => ({
	list_Incleanroom: [],
	list_Incleanroom_filter: [],
	list_Incleanroom_filter_slice: [],
	search: '',
	total_pages: 0,
	pages: 1,
	max_scope_page: 0,
  ...
  computed: {
    filtered_not_work() {
		let filter_data = this.list_Incleanroom.filter(p => {
	...
	methods: {
		async EmpLog () {
			await axios.get("http://localhost:5001/CombinedData")
				...
				.then((response) => {
					...
					console.log(this.list_Incleanroom);

					// const unique_id = [...new Set(this.list_Incleanroom.map(item => item.employeeInfo.empId))];
					const unique_biz = [...new Set(this.list_Incleanroom.map(item => item.biz))];
					this.all_biz = unique_biz

					const unique_process = [...new Set(this.list_Incleanroom.map(item => item.process))];
					this.all_process = unique_process
					
					const unique_course = [...new Set(this.list_Incleanroom.map(item => item.courseGroup[0]
					))];
					this.all_course_group = unique_course

					const unique_skill = [...new Set(this.list_Incleanroom.map(item => item.skillGroup[0]
					))];
		...
		format_data() {
			const data = this.list_Incleanroom
				// console.log(this.list_Incleanroom);
				const unique_id = [...new Set(this.list_Incleanroom.map(item => item.empId))];
				// const unique_biz = [...new Set(this.list_Incleanroom.map(item => item.EmployeeID))];
				// const unique_process = [...new Set(this.list_Incleanroom.map(item => item.EmployeeID))];
				// const unique_course = [...new Set(this.list_Incleanroom.map(item => item.EmployeeID))];
				// const unique_skill = [...new Set(this.list_Incleanroom.map(item => item.EmployeeID))];
				// console.log(test);
				...
				this.list_Incleanroom = last_data
		},
		set_caption(data) {
			this.list_Incleanroom_filter = data
			this.list_Incleanroom_filter_slice = data

			this.total_pages = Math.ceil(this.list_Incleanroom_filter.length/10)

			if (this.list_Incleanroom_filter.length > 10) {
		...
		set_data_per_page(select_page) {
			...
			if (select_page*10 > this.list_Incleanroom_filter.length) {
				this.max_scope_page = this.list_Incleanroom_filter.length
			}else {
				this.max_scope_page = select_page*10
			}

			this.list_Incleanroom_filter_slice = this.list_Incleanroom_filter.slice(((select_page-1)*10), select_page*10)
		},
		selected_biz (biz){
			console.log(biz);
